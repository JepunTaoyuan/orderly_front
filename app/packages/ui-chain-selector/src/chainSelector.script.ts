...
export const useChainSelectorScript = (
  ...
  const chains = useMemo(() => {
    ...
    // 在這裡加入排序邏輯
    const sortedMainnet = [...bridgeLessChains].sort((a, b) => {
      if (a.network_infos.chain_id === 56) return -1; // BNB Chain 排第一
      if (b.network_infos.chain_id === 56) return 1;
      return 0;
    });

    const sortedTestnet = [..._chains.testnet].sort((a, b) => {
      if (a.network_infos.chain_id === 97) return -1; // BSC Testnet 排第一
      if (b.network_infos.chain_id === 97) return 1;
      return 0;
    });
    return {
      mainnet: sortedMainnet.map((chain) => ({
        name: chain.network_infos.name,
        id: chain.network_infos.chain_id,
        lowestFee: chain.network_infos.bridgeless,
        isTestnet: false,
      })),
      testnet: sortedTestnet.map((chain) => ({
        name: chain.network_infos.name,
        id: chain.network_infos.chain_id,
        lowestFee: chain.network_infos.bridgeless,
        isTestnet: true,
