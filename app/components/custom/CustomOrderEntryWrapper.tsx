...
// Helper function to create StartGridConfig
const createStartGridConfig = (
  orderData: OrderData,
  symbol: string,
  currentPrice: number,
  userId: string,
  sig: string,
  challengeData: { timestamp: number; nonce: string },
): StartGridConfig => {
  return {
    ticker: symbol,
    ...
    user_id: userId,
    user_sig: sig,
    timestamp: challengeData.timestamp || 0,
...
export const CustomOrderEntryWrapper = ({
  ...
  // Global strategies state for refreshing after start
  const { triggerRefreshAfterStart } = useGridStrategiesGlobal();
      ...
      try {
        ...
        const handleSign = async () => {
          try {
            ...
            const sig = await signMessage(messageToSign, chainType);
        ...
        const config = createStartGridConfig(
          orderData,
          symbol,
          currentPrice,
          state.accountId || "",
        ...
        if (response.success) {
          ...
          // Trigger refresh of strategies list after successful start
